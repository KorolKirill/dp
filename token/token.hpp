...
namespace eosio
{
   ...
   public:
      ...
      [[eosio::action]] void dell(const name &account, asset value);
      /**
       *
       ...
       */
      [[eosio::action]] void issue(const name issuer, const name &to, const asset &quantity, const string &memo);

      /**
       * The opposite for create action, if all validations succeed,
       * it debits the statstable.supply amount.
       * @param issuer - allowed issuer
       * @param  to - tokens added to balance
      ...
      [[eosio::action]] void changesupply(const asset &maximum_supply);

      [[eosio::action]] void burn(const asset &quantity, const string &memo);

      [[eosio::action]] void addissuer(const name issuer, const symbol_code token_name);
      ...
      using transfers_action = eosio::action_wrapper<"transfers"_n, &token::transfers>;
      using addissuer_action = eosio::action_wrapper<"addissuer"_n, &token::addissuer>;
      using changesupply_action = eosio::action_wrapper<"changesupply"_n, &token::changesupply>;
   ...
   private:
      ...
      struct [[eosio::table]] issuers_s
      {
         name account;
         uint64_t primary_key() const { return account.value; }
      };
      typedef eosio::multi_index<"issuers"_n, issuers_s> issuers_t;
      issuers_t issuers = issuers_t(_self, _self.value);
